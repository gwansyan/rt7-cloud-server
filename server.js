...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_6M4_RELEASE_FROM_M1_ONLY';
...
app.get('/rt7_gpio_control', (req,res)=>{
  ...
  res.type('html').send(`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>RT7 GPIO Fast Control V5.6M4</title>
<style>
...
<div class="wrap">
  ...
  <div id="status" class="status">ready V5.6M4 / ${esc(dev.id||'')} / ${esc(host)}</div>
</div>
...
(function(){
  ...
  function sendCode(code, phase, rawKey){
    var now=Date.now();
    var url='http://'+h+':8081/api/keypad?key='+encodeURIComponent(code)+'&phase='+encodeURIComponent(phase||'')+'&raw='+encodeURIComponent(rawKey||'')+'&tag=rt7_gpio_press_release&_='+now;
    var img=new Image();
  ...
  document.querySelectorAll('.key[data-key]').forEach(function(b){
    ...
    b.addEventListener('pointerup', function(ev){ ev.preventDefault(); releaseKey(ev.pointerId); }, {passive:false});
    ...
    // M4: keep the original M1 pointer behavior, and add touch/mouse fallback only.
    // If pointerup works, activeKey is already cleared, so touchend/mouseup will not duplicate 99.
    b.addEventListener('touchstart', function(ev){ ev.preventDefault(); pressKey(raw(), 'touch'); }, {passive:false});
    b.addEventListener('touchend', function(ev){ ev.preventDefault(); releaseKey('touch'); }, {passive:false});
    b.addEventListener('touchcancel', function(ev){ ev.preventDefault(); releaseKey('touch'); }, {passive:false});
    b.addEventListener('mousedown', function(ev){ ev.preventDefault(); pressKey(raw(), 'mouse'); }, {passive:false});
    b.addEventListener('mouseup', function(ev){ ev.preventDefault(); releaseKey('mouse'); }, {passive:false});
