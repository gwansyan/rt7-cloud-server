...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_2H_LIVE_FRAME_RECOGNITION_FIX';
...
// V5.2H: for face recognition, use the newest live stream frame directly.
// This keeps the smooth V5.2C LAN/CLOUD stream path and avoids forcing ESP32 to stop/recapture.
function rt7GetFreshLiveFrameForFace_() {
  ensureDataDir();
  let buf = null;
  let source = 'none';
  let metaTime = '';
  let deviceId = '#1';
  let ip = '';
  if (latestStreamFrame && Buffer.isBuffer(latestStreamFrame) && latestStreamFrame.length > 800) {
    buf = latestStreamFrame;
    source = 'realtime_live_frame_memory';
    metaTime = liveStreamState && liveStreamState.time || '';
    deviceId = liveStreamState && liveStreamState.device_id || '#1';
    ip = liveStreamState && liveStreamState.ip || '';
  } else if (fs.existsSync(STREAM_FRAME_FILE)) {
    try {
      buf = fs.readFileSync(STREAM_FRAME_FILE);
      source = 'realtime_live_frame_file';
      const st = fs.statSync(STREAM_FRAME_FILE);
      metaTime = st.mtime.toISOString();
    } catch (_) {}
  }
  if (!buf || buf.length < 800) return null;
  const tMs = metaTime ? new Date(metaTime).getTime() : 0;
  const age = tMs ? Math.max(0, Date.now() - tMs) : null;
  const b64 = buf.toString('base64');
  return {
    b64,
    bytes: buf.length,
    snap_time: metaTime || nowIso(),
    snap_source: source,
    snap_hash: rt7QuickHash_(b64),
    snap_age_ms: age,
    snap_wait_ms: 0,
    snap_forced_realtime: false,
    snap_request_ws_sent: 0,
    snap_live_frame_fallback: false,
    snap_stale_warning: age !== null && age > 2500,
    meta: { ok:true, bytes:buf.length, time:metaTime || nowIso(), source, device_id:deviceId, ip, url:'/api/rt7/camera/latest.jpg' }
  };
}
function rt7SendWsJsonToEsp_(obj) {
  ...
  try {
    ...
    console.log('[FACE_API][V52H][WS_CMD_RELAY] type=' + safeString(obj && obj.type) + ' sent=' + n + ' open=' + seen);
  } catch (e) { console.warn('[FACE_API][V52H][WS_CMD_RELAY_ERR] ' + String(e && e.message || e)); }
  return n;
}
...
async function rt7ForceRealtimeSnapshot_() {
  ...
  const cmd = queueCommand({
    ...
    message:'V52H force ESP32 realtime face snapshot trigger + live frame fallback'
  });
  const wsSentA = rt7SendWsJsonToEsp_({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V52H', time:nowIso() });
  const wsSentB = rt7SendToEspIntercom_(JSON.stringify({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V52H', relay:'intercom_path', time:nowIso() }));
  ...
  console.log('[FACE_API][V52H][SNAPSHOT_REQUEST] request_id=' + requestId + ' ws_sent=' + wsSent + ' before_time=' + (beforeMeta.time||'') + ' before_source=' + (beforeMeta.source||''));
  ...
  for (let i=0; i<90; i++) {
    ...
    if (latest.bytes >= 800 && ((freshByTime && newerThanBefore) || (freshByAge && isLiveFrame))) {
      ...
      console.log('[FACE_API][V52H][SNAPSHOT_FRESH_OR_LIVE] request_id=' + requestId + ' wait_ms=' + latest.snap_wait_ms + ' bytes=' + latest.bytes + ' hash=' + latest.snap_hash + ' age_ms=' + latest.snap_age_ms + ' source=' + latest.snap_source + ' ws_sent=' + wsSent + ' fallback=' + latest.snap_live_frame_fallback);
      return latest;
    }
  }
  ...
  console.warn('[FACE_API][V52H][SNAPSHOT_REALTIME_TIMEOUT] request_id=' + requestId + ' wait_ms=' + (Date.now() - startMs) + ' ws_sent=' + wsSent + ' last_hash=' + (stale && stale.snap_hash || '') + ' last_age_ms=' + (stale && stale.snap_age_ms || '') + ' last_source=' + (stale && stale.snap_source || ''));
  return {
...
function rt7FaceGateCheck_(latest) {
  ...
  console.log('[RT7_FACE_GATE][TOGGLE][V52H] enabled=' + gate.enabled + ' pass=' + gate.pass + ' bytes=' + gate.bytes + ' age_ms=' + gate.age_ms + ' reason=' + gate.reason);
  return gate;
}
...
function rt7RealFaceCountDetect_(latest) {
  const empty = {
    ...
    summary:'目前畫面未偵測到人臉。', raw:'LOCAL_REAL_FACE_COUNT_V52H'
  };
  let img;
  ...
  return {
    ...
    raw:'LOCAL_REAL_FACE_COUNT_V52H features=' + best.features + ' skin=' + skinTotal + ' blob=' + JSON.stringify({w:best.w,h:best.h,area:best.area,fill:best.fill,dark_ratio:best.dark_ratio}),
...
function rt7RailwayFaceCompare_(latest, refs, detect) {
  ...
  if (!cur.ok) return { known_face:false, matched_name:'', confidence:0, reason:cur.reason || 'CURRENT_EMBED_FAIL', summary:'目前照片無法建立人臉特徵。', raw:'RAILWAY_FACE_MATCH_V52H' };
  let best = null;
  for (const f of refs || []) {
  ...
  if (!best) return { known_face:false, matched_name:'', confidence:0, reason:'NO_VALID_REFERENCE_EMBEDDING', summary:'註冊照片無法建立可比對的人臉特徵。', raw:'RAILWAY_FACE_MATCH_V52H' };
  ...
  return {
    ...
    raw:'RAILWAY_FACE_MATCH_V52H cosine=' + best.cosine + ' name=' + best.name,
    best
  };
}
...
async function rt7FaceMatchLatest_() {
  console.log('[FACE_API][V52H] /api/rt7/face/match ENTER live_frame_direct=1 force_capture=0');
  const latest = rt7GetFreshLiveFrameForFace_();
  if (!latest || !latest.b64 || (latest.snap_age_ms !== null && latest.snap_age_ms > 5000)) {
    const stale = latest || rt7GetLatestWithMeta_() || null;
    const fail = {
      ok:true, version:SERVER_VERSION, api_entered:true, api_path:'/api/rt7/face/match', type:'face_match', stage:'LIVE_FRAME_REQUIRED', engine:'railway_local', gpt_used:false,
      ...
      summary:'未取得新鮮 live frame，已停止：請先按開始影像，讓串流穩定後再辨識。',
      ...
      debug_text:'LIVE_FRAME_DIRECT=YES NO_FRESH_LIVE_FRAME STALE_HASH=' + (stale && stale.snap_hash || '') + ' AGE=' + (stale && stale.snap_age_ms || ''),
      time:nowIso()
    };
    cloudState.last_face_match = fail; broadcast('face_match', fail);
    console.warn('[FACE_API][V52H] stop before match: NO_FRESH_LIVE_FRAME ws_sent=' + fail.snap_request_ws_sent + ' wait_ms=' + fail.snap_wait_ms + ' stale_hash=' + fail.snap_hash + ' stale_age_ms=' + fail.snap_age_ms);
    return fail;
  }
  try { fs.writeFileSync(FACE_DEBUG_SNAPSHOT_FILE, Buffer.from(latest.b64, 'base64')); } catch (e) { console.warn('[FACE_API][V52H] save face debug snapshot failed', e && e.message || e); }
  ...
  if (cloudState.face_gate_enabled && !gate.pass) {
    ...
    console.log('[FACE_API][V52H] FACE_GATE_SKIP hash=' + latest.snap_hash + ' reason=' + gate.reason);
    return skip;
  }

  const detect = await rt7DetectFaceOnly_(latest);
  console.log('[FACE_API][V52H] detect face_found=' + detect.face_found + ' count=' + detect.face_count + ' box=' + JSON.stringify(detect.face_box) + ' ratio=' + detect.face_ratio + ' reason=' + detect.reason + ' hash=' + latest.snap_hash);
...
app.post('/api/rt7/face_gate/toggle', (req,res) => {
  ...
  console.log('[RT7_FACE_GATE][TOGGLE][V52H] set enabled=' + cloudState.face_gate_enabled);
  res.json({ ok:true, version:SERVER_VERSION, enabled:!!cloudState.face_gate_enabled, last_face_gate:cloudState.last_face_gate || null });
});
...
app.get('/api/rt7/face/snapshot_trigger_test', (req,res)=>{
  ...
  const wsSent = rt7SendWsJsonToEsp_({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V52H', manual:true, time:nowIso() });
