...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_4H_FACE_GATE_MIN_FACE_AREA';
...
function rt7SendWsJsonToEsp_(obj) {
  ...
  try {
    ...
    console.log('[FACE_API][V54H][WS_CMD_RELAY] type=' + safeString(obj && obj.type) + ' sent=' + n + ' open=' + seen);
  } catch (e) { console.warn('[FACE_API][V54H][WS_CMD_RELAY_ERR] ' + String(e && e.message || e)); }
  return n;
}
...
async function rt7ForceRealtimeSnapshot_() {
  ...
  } catch (e) { console.warn('[FACE_API][V54H][CLEAR_PENDING_WARN] ' + String(e && e.message || e)); }
  ...
  const cmd = queueCommand({
    ...
    message:'V54H single-shot face snapshot trigger; duplicate face_snapshot_now suppressed'
  });
  const wsSentA = rt7SendWsJsonToEsp_({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V54H', time:nowIso() });
  const wsSentB = rt7SendToEspIntercom_(JSON.stringify({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V54H', relay:'intercom_path', time:nowIso() }));
  ...
  console.log('[FACE_API][V54H][SNAPSHOT_REQUEST] request_id=' + requestId + ' ws_sent=' + wsSent + ' before_time=' + (beforeMeta.time||'') + ' before_source=' + (beforeMeta.source||''));
  ...
  for (let i=0; i<90; i++) {
    ...
    if (latest.bytes >= 800 && ((freshByTime && newerThanBefore) || (freshByAge && isLiveFrame))) {
      ...
      console.log('[FACE_API][V54H][SNAPSHOT_FRESH_OR_LIVE] request_id=' + requestId + ' wait_ms=' + latest.snap_wait_ms + ' bytes=' + latest.bytes + ' hash=' + latest.snap_hash + ' age_ms=' + latest.snap_age_ms + ' source=' + latest.snap_source + ' ws_sent=' + wsSent + ' fallback=' + latest.snap_live_frame_fallback);
      return latest;
    }
  }
  ...
  console.warn('[FACE_API][V54H][SNAPSHOT_REALTIME_TIMEOUT] request_id=' + requestId + ' wait_ms=' + (Date.now() - startMs) + ' ws_sent=' + wsSent + ' last_hash=' + (stale && stale.snap_hash || '') + ' last_age_ms=' + (stale && stale.snap_age_ms || '') + ' last_source=' + (stale && stale.snap_source || ''));
  return {
...
function rt7RailwayFaceCompare_(latest, refs, detect) {
  ...
  if (!cur.ok) return { known_face:false, matched_name:'', confidence:0, reason:cur.reason || 'CURRENT_EMBED_FAIL', summary:'目前照片無法建立人臉特徵。', raw:'RAILWAY_FACE_MATCH_V54H', match_ms:Date.now()-t0, cache_mode:'FAST_CACHE' };
  let best = null;
  ...
  if (!best) return { known_face:false, matched_name:'', confidence:0, reason:'NO_VALID_REFERENCE_EMBEDDING', summary:'註冊照片無法建立可比對的人臉特徵。', raw:'RAILWAY_FACE_MATCH_V54H', match_ms:matchMs, cache_mode:'FAST_CACHE', cache_hits:cacheHits, cache_miss:cacheMiss, compared };
  ...
  return {
    ...
    raw:'RAILWAY_FACE_MATCH_V54H cosine=' + best.cosine + ' name=' + best.name + ' match_ms=' + matchMs + ' cache_hits=' + cacheHits + ' cache_miss=' + cacheMiss,
    best,
    match_ms:matchMs,
...
async function rt7FaceMatchLatestCore_(providedLatest, opt) {
  ...
  console.log('[FACE_API][V54H] /api/rt7/face/match CORE ENTER detect_first=1 force_realtime=' + (providedLatest ? '0' : '1') + ' auto_face_gate=' + (autoMode ? '1' : '0'));
  ...
  if (!latest || latest.realtime_failed || !latest.b64) {
    ...
    console.warn('[FACE_API][V54H] stop before AI: NO_REALTIME_SNAPSHOT ws_sent=' + fail.snap_request_ws_sent + ' wait_ms=' + fail.snap_wait_ms + ' stale_hash=' + fail.snap_hash + ' stale_age_ms=' + fail.snap_age_ms);
    return fail;
  }
  try { fs.writeFileSync(FACE_DEBUG_SNAPSHOT_FILE, Buffer.from(latest.b64, 'base64')); } catch (e) { console.warn('[FACE_API][V54H] save face debug snapshot failed', e && e.message || e); }
  ...
  if (cloudState.face_gate_enabled && !gate.pass) {
    ...
    console.log('[FACE_API][V54H] FACE_GATE_SKIP hash=' + latest.snap_hash + ' reason=' + gate.reason);
    return skip;
  }

  const detect = await rt7DetectFaceOnly_(latest);
  console.log('[FACE_API][V54H] detect face_found=' + detect.face_found + ' count=' + detect.face_count + ' box=' + JSON.stringify(detect.face_box) + ' ratio=' + detect.face_ratio + ' reason=' + detect.reason + ' hash=' + latest.snap_hash);
  ...
  try { if (rt7RefreshFaceEmbeddingCaches_(faces)) rt7SaveFaces_(faces); } catch(e) { console.warn('[FACE_API][V54H][CACHE_REFRESH_WARN] ' + String(e && e.message || e)); }
...
async function rt7FaceMatchLatest_() {
  ...
  if (rt7FaceMatchServerBusy_) {
    ...
    console.log('[FACE_API][V54H][SINGLE_SHOT_BUSY] reuse last result, age_ms=' + (now - rt7FaceMatchServerStartedMs_));
    return out;
  }
  if (rt7FaceMatchServerLastResult_ && (now - rt7FaceMatchServerStartedMs_) < 2200) {
    ...
    console.log('[FACE_API][V54H][SINGLE_SHOT_COOLDOWN] reuse recent result, age_ms=' + (now - rt7FaceMatchServerStartedMs_));
    return out;
  }
...
function rt7EnrollHandler_(req,res){
  ...
  } catch(e) { console.warn('[FACE_API][V54H][ENROLL_CACHE_WARN] ' + String(e && e.message || e)); }
  faces.unshift(face);
...
function rt7StartFaceGateAutoMatch_(reason) {
  ...
  if (!cloudState.face_gate_auto_enabled) {
    console.log('[RT7_FACE_GATE_AUTO][V54H][SKIP] auto_disabled ai=' + (cloudState.ai_enabled?1:0) + ' auto=' + (cloudState.face_gate_auto_enabled?1:0));
    return false;
  }
  if (cloudState.face_gate_auto_busy) {
    console.log('[RT7_FACE_GATE_AUTO][V54H][SKIP] busy reason=' + safeString(reason));
    return false;
  }
  ...
  if (cloudState.face_gate_auto_last_ms && (now - cloudState.face_gate_auto_last_ms) < cd) {
    console.log('[RT7_FACE_GATE_AUTO][V54H][SKIP] cooldown remain=' + (cd - (now - cloudState.face_gate_auto_last_ms)) + 'ms');
    return false;
  }
  ...
  setTimeout(async () => {
    try {
      ...
      console.log('[RT7_FACE_GATE_AUTO][V54H][MATCH_START] hash=' + latest.snap_hash + ' bytes=' + latest.bytes + ' age=' + latest.snap_age_ms + 'ms reason=' + safeString(reason));
      ...
      console.log('[RT7_FACE_GATE_AUTO][V54H][MATCH_DONE] known=' + (r && r.known_face ? 1 : 0) + ' conf=' + (r && r.confidence || 0) + ' reason=' + (r && r.reason || ''));
    } catch (e) {
      ...
      console.warn('[RT7_FACE_GATE_AUTO][V54H][MATCH_ERR] ' + String(e && e.message || e));
    } finally {
...
app.post('/api/rt7/phase6a_fix2/motion/event', (req,res)=>{
  ...
  } catch (e) { console.warn('[RT7_FACE_GATE_AUTO][V54H][MOTION_EVENT_ERR] ' + String(e && e.message || e)); }
...
app.get('/api/rt7/face/snapshot_trigger_test', (req,res)=>{
  ...
  const wsSent = rt7SendWsJsonToEsp_({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V54H', manual:true, time:nowIso() });
