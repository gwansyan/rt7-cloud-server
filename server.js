...
(function(){
  ...
  function rt7RxPlayPcm(ab){
    try{
      ...
      // V5.5C: fixed 3fps-low-delay-balance queue like the Node-RED player; do not let adaptive
...
function rt7IcatchRealtimeMjpegPage_() {
  ...
  const esc = (v) => String(v === undefined || v === null ? '' : v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const card = `<div class="cam live"><div class="camTop"><b>${esc(ch1.id)} / ${esc(ch1.name||'CH01 大門')}</b><span id="s_CH01">讀取中</span></div><div class="view"><img id="img_CH01" src="/api/rt7/dvr/bridge/stream/CH01?fps=3&v=62h&first=${Date.now()}" onerror="document.getElementById('s_CH01').textContent='MJPEG_ERROR';"></div><div class="meta">True MJPEG: <code>/api/rt7/dvr/bridge/stream/CH01?fps=3</code><br><span id="m_CH01">MJPEG 串流中；每 3 秒檢查 Bridge 狀態，不重載畫面。</span></div><div class="btns"><button onclick="reloadMjpeg()">重連串流</button><button onclick="latestStill()">看單張</button><button onclick="ai('CH01')">AI 辨識</button></div></div>`;
...
</style></head><body><div class="top"><a href="/rt7_dvr_ai_platform">← DVR平台</a><h1>RT7 V6.2H iCATCH 3FPS低延遲平衡</h1><a href="/api/rt7/dvr/bridge/status">JSON</a></div><div class="wrap"><div class="notice"><b>V6.2H：</b>3FPS 低延遲平衡版：保留 V6.2G 安全解碼避免藍綠屏，Bridge 只保留最新 frame、上傳塞車就丟舊 frame，降低排隊延遲。MJPEG 端點保留上一張有效影像，ONLINE/OFFLINE 30 秒內不誤判。CH02~CH04 仍停用。</div><div class="grid">${card}${disabled}</div><div class="card"><h2>Bridge 狀態</h2><div id="log" class="log">讀取中...</div></div></div><script>
...
function reloadMjpeg(){const img=document.getElementById('img_CH01'); if(img){img.src='/api/rt7/dvr/bridge/stream/CH01?fps=3&v=62h&reload='+Date.now(); setS('RECONNECT','ok');}}
