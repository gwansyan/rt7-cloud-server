...
const SERVER_VERSION = 'RT7_V6_2H_ICATCH_3FPS_LOW_DELAY_BALANCE';
...
(function(){
  ...
  function rt7RxPlayPcm(ab){
    try{
      ...
      // V5.5C: fixed direct-lan-view queue like the Node-RED player; do not let adaptive
...
// ---------------- V6.0A DVR / iCATCH Protocol Bridge ----------------
...
const RT7_DVR_BRIDGE_DIR = path.join(DATA_DIR, 'rt7_dvr_bridge_frames');
const RT7_DVR_BRIDGE_TOKEN = process.env.RT7_DVR_BRIDGE_TOKEN || 'rt7-dvr-bridge';
...
async function rt7DvrProbeCamera_(cam) {
  ...
  for (const url of Array.from(new Set(candidates))) {
    const r = await rt7HttpGetBuffer_(url, { username:cam.username, password:cam.password, timeout:1200, maxBytes:512*1024 });
  ...
  return { ok:false, error:'NO_WORKING_HTTP_SNAPSHOT_FOUND', tried, hint:'SoCatch App 可看影像不代表 DVR 一定開放 HTTP Snapshot；手機瀏覽器直接開 192.168.0.123 出現 ERR_EMPTY_RESPONSE 時，代表它不是一般網頁服務。請使用 RT7 LAN Bridge/FFmpeg Bridge，把內網 DVR 畫面主動上傳到 Railway。' };
}

function rt7DvrBridgeEnsureDir_() {
  ensureDataDir();
  if (!fs.existsSync(RT7_DVR_BRIDGE_DIR)) fs.mkdirSync(RT7_DVR_BRIDGE_DIR, { recursive:true });
}
function rt7DvrBridgeFramePath_(id) {
  rt7DvrBridgeEnsureDir_();
  id = safeString(id || '').toUpperCase().replace(/[^A-Z0-9_-]/g, '') || 'CH01';
  return path.join(RT7_DVR_BRIDGE_DIR, id + '.jpg');
}
function rt7DvrBridgeMetaPath_(id) {
  rt7DvrBridgeEnsureDir_();
  id = safeString(id || '').toUpperCase().replace(/[^A-Z0-9_-]/g, '') || 'CH01';
  return path.join(RT7_DVR_BRIDGE_DIR, id + '.json');
}
...
function rt7DvrDetectVideoLossJpeg_(buf) {
  const out = { video_loss:false, reason:'', blue_ratio:0, avg_r:0, avg_g:0, avg_b:0, samples:0 };
  if (!buf || buf.length < 128) return out;
  let img;
  try { img = jpeg.decode(buf, { useTArray:true, maxMemoryUsageInMB:80 }); } catch (_) { out.reason='JPEG_DECODE_FAIL'; return out; }
...
function rt7DvrBridgeLatest_(id) {
  ...
  try {
    ...
    return Object.assign({}, meta, { ok:true, id, file, bytes:st.size, updated_at:new Date(st.mtimeMs).toISOString(), age_ms, online:age_ms < 30000, stale: age_ms >= 30000 });
  } catch (_) {
    return Object.assign({}, meta, { ok:false, id, online:false, error:'NO_BRIDGE_FRAME' });
  }
}
function rt7DvrBridgeStatus_() {
  return rt7DvrReadCameras_().map(c => rt7DvrBridgeLatest_(c.id));
}
async function rt7DvrFetchSnapshotWithBridge_(cam) {
  const latest = rt7DvrBridgeLatest_(cam && cam.id);
  if (latest.ok && latest.online && latest.file) {
    try {
      const buffer = fs.readFileSync(latest.file);
      if (buffer && buffer.length > 4) return { ok:true, source:'bridge', buffer, status:200, url:'/api/rt7/dvr/bridge/latest/' + encodeURIComponent(cam.id), bridge:latest };
    } catch (_) {}
  }
  const r = await rt7DvrFetchSnapshot_(cam);
  return Object.assign(r || {}, { source:(r && r.ok) ? 'http_snapshot' : 'http_snapshot_failed', bridge:latest });
}
...
<div class="card"><h2>Bridge 狀態</h2><div id="bridge" class="log">讀取中...</div></div><div class="card"><h2>測試結果</h2><div id="log" class="log">等待測試...</div></div>
</div><script>
...
function rt7DvrServeMjpeg_(req, res, id) {
  ...
  res.writeHead(200, {
    ...
    'Expires':'0',
    'X-Accel-Buffering':'no',
    'X-RT7-DVR-MJPEG':'V6_2H_ICATCH_3FPS_LOW_DELAY_BALANCE',
    'X-RT7-DVR-Camera': id
  });
  let closed = false;
  let busy = false;
...
function rt7IcatchRealtimeMjpegPage_() {
  ...
  const esc = (v) => String(v === undefined || v === null ? '' : v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const card = `<div class="cam live"><div class="camTop"><b>${esc(ch1.id)} / ${esc(ch1.name||'CH01 大門')}</b><span id="s_CH01">讀取中</span></div><div class="view"><img id="img_CH01" src="/api/rt7/dvr/bridge/stream/CH01?fps=3&v=62i&first=${Date.now()}" onerror="document.getElementById('s_CH01').textContent='MJPEG_ERROR';"></div><div class="meta">True MJPEG: <code>/api/rt7/dvr/bridge/stream/CH01?fps=3</code><br><span id="m_CH01">MJPEG 串流中；每 3 秒檢查 Bridge 狀態，不重載畫面。</span></div><div class="lanbox"><b>LAN直連：</b><input id="lanUrl" placeholder="http://192.168.0.55:8787" value=""><button onclick="useLanDirect()">啟用LAN直連</button><button onclick="useRailway()">回Railway</button><div id="lanHint" class="hint">手機與 Bridge 電腦同一 Wi-Fi/LAN 時，填 Bridge 電腦 IP，可避開 Railway 中轉。</div></div><div class="btns"><button onclick="reloadMjpeg()">重連串流</button><button onclick="latestStill()">看單張</button><button onclick="ai('CH01')">AI 辨識</button></div></div>`;
...
function railwayStreamUrl(){return '/api/rt7/dvr/bridge/stream/CH01?fps=3&v=62i&reload='+Date.now();}
function reloadMjpeg(){const img=document.getElementById('img_CH01'); if(img){img.src=railwayStreamUrl(); setS('RECONNECT','ok');}}
function useLanDirect(){const input=document.getElementById('lanUrl'); let base=(input&&input.value||'').trim().replace(/\/+$/,''); if(!base){alert('請輸入 Bridge 電腦網址，例如 http://192.168.0.55:8787'); return;} localStorage.setItem('rt7_lan_bridge_url',base); const img=document.getElementById('img_CH01'); if(img){img.src=base+'/stream/CH01.mjpg?ts='+Date.now(); setS('LAN DIRECT','ok');} const h=document.getElementById('lanHint'); if(h) h.textContent='LAN直連中：'+base+'；若瀏覽器阻擋 HTTP 混合內容，請直接開 '+base+'/direct';}
function useRailway(){localStorage.removeItem('rt7_lan_bridge_url'); reloadMjpeg(); const h=document.getElementById('lanHint'); if(h) h.textContent='已切回 Railway MJPEG。';}
...
async function status(){const r=await fetch('/api/rt7/dvr/bridge/status?ts='+Date.now()).then(r=>r.json()).catch(e=>({ok:false,error:String(e)})); document.getElementById('log').textContent=JSON.stringify(r,null,2); const ch=(r.cameras||[]).find(c=>c.id==='CH01'); const m=document.getElementById('m_CH01'); if(ch && (ch.online || (ch.age_ms!=null && ch.age_ms<30000))){lastGoodAt=Date.now(); setS('ONLINE','ok'); if(m) m.textContent='串流狀態：ONLINE / bytes='+(ch.bytes||0)+' / age_ms='+(ch.age_ms||0)+' / direct-lan-view';} else if(Date.now()-lastGoodAt>30000){setS('OFFLINE','bad'); if(m) m.textContent='30秒內沒有收到 Bridge 新影像，仍保留目前畫面：'+new Date().toLocaleTimeString();}}
const savedLan=localStorage.getItem('rt7_lan_bridge_url')||''; const lanInput=document.getElementById('lanUrl'); if(lanInput) lanInput.value=savedLan; if(savedLan){setTimeout(useLanDirect,300);} status(); setInterval(status,3000);
</script></body></html>`;
}
...
app.get('/api/rt7/dvr/snapshot/:id', rt7RequireLogin_, async (req,res) => {
  ...
  const r = await rt7DvrFetchSnapshotWithBridge_(cam);
  if (!r.ok) return res.status(502).type('text/plain').send((r.error || 'DVR_FETCH_FAIL') + '\n' + (r.hint || '') + '\nBRIDGE=' + JSON.stringify(r.bridge || {}));
...
app.post('/api/rt7/dvr/ai/recognize/:id', rt7RequireLogin_, async (req,res) => {
  ...
  if (!cam) return res.status(404).json({ ok:false, error:'CAMERA_NOT_FOUND' });
  const r = await rt7DvrFetchSnapshotWithBridge_(cam);
  if (!r.ok) return res.status(502).json({ ok:false, error:r.error || 'DVR_FETCH_FAIL', hint:r.hint || '', camera:cam.id, bridge:r.bridge || {} });
  // V5.9B 先完成 DVR 影像接入；若要接既有人臉比對，可在此把 r.buffer 送入 Face Match pipeline。
  const out = { ok:true, camera:cam.id, source:r.source || 'unknown', bytes:r.buffer.length, message:'DVR_SNAPSHOT_OK_AI_PIPELINE_READY', time:nowIso() };
...
app.get('/api/rt7/dvr/bridge/status', rt7RequireLogin_, (req,res) => {
  res.json({ ok:true, version:SERVER_VERSION, token_set:!!RT7_DVR_BRIDGE_TOKEN, cameras:rt7DvrBridgeStatus_() });
});
app.get('/api/rt7/dvr/bridge/latest/:id', rt7RequireLogin_, (req,res) => {
  const latest = rt7DvrBridgeLatest_(req.params.id);
  if (!latest.ok || !latest.file) return res.status(404).type('text/plain').send('NO_BRIDGE_FRAME');
  res.setHeader('Cache-Control','no-store');
  res.setHeader('Content-Type','image/jpeg');
...
app.post('/api/rt7/dvr/bridge/upload/:id', express.raw({ type:['image/jpeg','image/jpg','application/octet-stream'], limit:'3mb' }), (req,res) => {
  ...
  const id = safeString(req.params.id || '').toUpperCase().replace(/[^A-Z0-9_-]/g, '');
  if (!id) return res.status(400).json({ ok:false, error:'BAD_CAMERA_ID' });
  const buf = Buffer.isBuffer(req.body) ? req.body : Buffer.alloc(0);
  if (buf.length < 4 || buf[0] !== 0xff || buf[1] !== 0xd8) return res.status(400).json({ ok:false, error:'NOT_JPEG', bytes:buf.length });
  const videoLoss = rt7DvrDetectVideoLossJpeg_(buf);
  if (videoLoss.video_loss) {
    const meta = Object.assign(rt7DvrBridgeReadMeta_(id) || {}, {
      ok:true, id, skipped:true, reason:'VIDEO_LOSS_FRAME_FILTERED', filtered_at:nowIso(), last_rejected_bytes:buf.length, video_loss:videoLoss,
      source_ip:clientIp(req), user_agent:safeString(req.headers['user-agent'] || ''), single_source:true
    });
  ...
  const meta = { ok:true, id, bytes:buf.length, uploaded_at:nowIso(), source_ip:clientIp(req), user_agent:safeString(req.headers['user-agent'] || ''), single_source:true, video_loss_filtered:false };
  rt7WriteJsonFile_(rt7DvrBridgeMetaPath_(id), meta);
