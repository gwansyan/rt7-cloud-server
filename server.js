...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V4_9S_LAN_STREAM_FAST_DOOROPEN_DIRECT';
...
<title>RT7 Cloud Original UI V4.9S</title>
<style>
...
(function(){
  ...
  function openDoorLanDirect(){
    setAnswer('LAN 直連開門命令送出中...');
    setDebug('LAN direct door open to ESP32');
    var done=false;
    var url='http://'+ip+'/api/door/open?_lan_door='+Date.now();
    var beacon=new Image();
    var t=setTimeout(function(){
      if(done) return;
      done=true;
      setAnswer('LAN 直連開門已送出（ESP32 會本機執行；若無反應請切 CLOUD 模式）');
      setDebug('LAN direct door open timeout assumed sent');
    },900);
    beacon.onload=function(){
      if(done) return;
      done=true;
      clearTimeout(t);
      setAnswer('LAN 直連開門已送出');
      setDebug('LAN direct door open image onload');
    };
    beacon.onerror=function(){
      if(done) return;
      done=true;
      clearTimeout(t);
      // ESP32 回 JSON 不是圖片，onerror 也代表 GET 已送出；不可當失敗。
      setAnswer('LAN 直連開門已送出');
      setDebug('LAN direct door open image onerror but request sent');
    };
    beacon.src=url;
  }
  bind('btnOpenDoor', async function(){
    ...
    setAnswer('CLOUD 開門命令送出中...');
    try{
      var r=await j('/api/rt7/door/open?device_id='+encodeURIComponent('#1'));
      setAnswer((r.note||r.message||'開門命令已送出'));
      setDebug('cloud door open '+JSON.stringify(r).slice(0,160));
    }catch(e){ setAnswer('開門失敗：'+e.message); }
  });
