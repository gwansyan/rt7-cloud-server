...
function appendEvent(event) {
  ...
  // V5.6B: keep Railway storage small. Text logs are retained, historical snapshots are not duplicated.
  try {
    const maxBytes = parseInt(process.env.RT7_EVENT_LOG_MAX_BYTES || '5242880', 10); // 5 MB
    const keepLines = parseInt(process.env.RT7_EVENT_LOG_KEEP_LINES || '5000', 10);
    const st = fs.statSync(EVENT_LOG);
    if (st.size > maxBytes) {
      const lines = fs.readFileSync(EVENT_LOG, 'utf8').split(/\r?\n/).filter(Boolean);
      fs.writeFileSync(EVENT_LOG, lines.slice(Math.max(0, lines.length - keepLines)).join('\n') + '\n', 'utf8');
    }
  } catch (_) {}
  return row;
}
...
<a class="btn green" href="/rt7_device_manager">設備管理</a>
<a class="btn gray" href="/rt7_event_log">事件紀錄</a>
...
// ---------- V5.6B Device Manager + Event Log ----------
function rt7EventLabel_(type) {
  const t = safeString(type);
  const map = {
    doorbell:'門鈴', door_open:'開門', door_open_queue:'開門', command:'命令', command_ack:'命令完成',
    face_match:'人臉辨識', face_gate_auto_snapshot:'人臉快照', face_enroll:'人臉註冊',
    face_match_detect_no_face:'未偵測到人臉', motion:'移動/人臉觸發', device_register:'設備上線',
    devices_save:'設備儲存', intercom_begin:'對講開始', intercom_end:'對講結束', plugin_set:'Plugin'
  };
  return map[t] || t || '事件';
}
function rt7FilterEvents_(req) {
  const limit = Math.min(parseInt(req.query.limit || '500', 10) || 500, 3000);
  const type = safeString(req.query.type || '').trim();
  const dev = safeString(req.query.device_id || req.query.device || '').trim();
  const q = safeString(req.query.q || '').trim().toLowerCase();
  let events = readEvents(Math.max(limit * 3, limit));
  if (type) events = events.filter(e => safeString(e.type) === type);
  if (dev) events = events.filter(e => safeString(e.device_id || e.device || e.requested_device_id) === dev);
  if (q) events = events.filter(e => JSON.stringify(e).toLowerCase().includes(q));
  return events.slice(Math.max(0, events.length - limit));
}
app.get('/api/rt7/events/latest', (req,res)=>res.json({ ok:true, version:SERVER_VERSION, events:rt7FilterEvents_(req), devices:readDevices() }));
app.get('/api/rt7/events/clear', (req,res)=>{
  ensureDataDir();
  fs.writeFileSync(EVENT_LOG, '', 'utf8');
  const ev = appendEvent({ type:'events_clear', message:'event log cleared from V5.6B UI' });
  res.json({ ok:true, version:SERVER_VERSION, event:ev });
});
app.post('/api/rt7/events/clear', (req,res)=>{
  ensureDataDir();
  fs.writeFileSync(EVENT_LOG, '', 'utf8');
  const ev = appendEvent({ type:'events_clear', message:'event log cleared from V5.6B UI' });
  res.json({ ok:true, version:SERVER_VERSION, event:ev });
});
app.get('/api/rt7/device/state', (req,res)=>res.json({ ok:true, version:SERVER_VERSION, devices:readDevices(), current_device:cloudState.current_device_id || '#1' }));
app.post('/api/rt7/device/set', (req,res)=>{
  const body=req.body||{};
  const id=safeString(body.id||body.device_id||req.query.id||req.query.device_id||'#1').trim() || '#1';
  const devices=readDevices();
  let d=devices.find(x=>x.id===id);
  if(!d){ d={id, name:id, ip:'', enabled:true}; devices.push(d); }
  if(body.name!==undefined || req.query.name!==undefined) d.name=safeString(body.name!==undefined?body.name:req.query.name);
  if(body.ip!==undefined || req.query.ip!==undefined) d.ip=safeString(body.ip!==undefined?body.ip:req.query.ip).replace(/^https?:\/\//,'').replace(/\/$/,'');
  if(body.enabled!==undefined || req.query.enabled!==undefined) d.enabled=!/^(0|false|off|no)$/i.test(safeString(body.enabled!==undefined?body.enabled:req.query.enabled));
  saveDevices(devices);
  const ev=appendEvent({type:'devices_save', device_id:id, device_name:d.name, ip:d.ip, message:'device updated from manager'});
  res.json({ok:true, version:SERVER_VERSION, device:d, devices, event:ev});
});
app.post('/api/rt7/devices/save', (req,res)=>{
  const devices=saveDevices(req.body?.devices || req.body || []);
  const ev=appendEvent({type:'devices_save', device_count:devices.length, message:'devices saved from V5.6B manager'});
  res.json({ok:true, version:SERVER_VERSION, devices, event:ev});
});

app.get('/rt7_device_manager', (req,res)=>{
  res.type('html').send(htmlShell('RT7 Device Manager V5.6B', `${baseCss}
<header class="top"><h1>RT7 設備管理 V5.6B</h1><p>#1~#4 影像對講門禁 名稱/IP 設定</p></header>
<main class="wrap">
<section class="card"><a class="btn" href="/rt7_cloud_original_ui_doorbell">手機門禁頁</a><a class="btn gray" href="/rt7_event_log">事件紀錄</a><button class="btn green" onclick="saveAll()">儲存設定</button><button class="btn gray" onclick="loadDevices()">重新讀取</button></section>
<section class="card"><h2>設備清單</h2><p class="muted">IP 請填 ESP32 在內網的 IP，例如 192.168.0.179。Railway 手機頁會讀取此清單。</p><div style="overflow:auto"><table><thead><tr><th>啟用</th><th>Device</th><th>名稱</th><th>IP</th><th>測試</th></tr></thead><tbody id="rows"></tbody></table></div></section>
<section class="card"><h3>狀態</h3><pre id="status" class="status">loading...</pre></section>
</main>
<script>
function $(id){return document.getElementById(id)}
function esc(s){return String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));}
async function j(url,opt){const r=await fetch(url,Object.assign({cache:'no-store'},opt||{}));const t=await r.text();try{return JSON.parse(t)}catch(e){return{ok:r.ok,status:r.status,raw:t}}}
let devices=[];
function norm(list){const ids=['#1','#2','#3','#4'];return ids.map((id,i)=>{let d=(list||[]).find(x=>x.id===id)||{};return {id,name:d.name||(['RT7 ESP32-S3-CAM','影像對講','RT7 S3-CAM-A','RT7 S3-CAM-B'][i]),ip:d.ip||'',enabled:d.enabled!==false};});}
async function loadDevices(){const r=await j('/api/devices');devices=norm(r.devices||[]);render();$('status').textContent=JSON.stringify(r,null,2)}
function render(){ $('rows').innerHTML=devices.map((d,i)=>'<tr><td><input type="checkbox" id="en'+i+'" '+(d.enabled?'checked':'')+'></td><td><b>'+esc(d.id)+'</b></td><td><input id="name'+i+'" value="'+esc(d.name)+'" style="width:160px"></td><td><input id="ip'+i+'" value="'+esc(d.ip)+'" placeholder="192.168.0.xxx" style="width:160px"></td><td><button class="btn gray" onclick="testCam('+i+')">測試影像</button></td></tr>').join(''); }
function collect(){return devices.map((d,i)=>({id:d.id,name:$('name'+i).value.trim()||d.id,ip:$('ip'+i).value.trim(),enabled:$('en'+i).checked}));}
async function saveAll(){const list=collect();const r=await j('/api/devices/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({devices:list})});devices=norm(r.devices||list);render();$('status').textContent=JSON.stringify(r,null,2);alert('已儲存設備設定');}
function testCam(i){const ip=$('ip'+i).value.trim(); if(!ip){alert('請先填 IP');return;} window.open('http://'+ip+'/api/camera/stream','_blank');}
loadDevices();
</script>`));
});

app.get('/rt7_event_log', (req,res)=>{
  res.type('html').send(htmlShell('RT7 Event Log V5.6B', `${baseCss}
<header class="top"><h1>RT7 事件紀錄 V5.6B</h1><p>開門 / 門鈴 / 人臉辨識 / 對講 / 設備事件</p></header>
<main class="wrap">
<section class="card"><a class="btn" href="/rt7_cloud_original_ui_doorbell">手機門禁頁</a><a class="btn gray" href="/rt7_device_manager">設備管理</a><button class="btn gray" onclick="loadLog()">重新讀取</button><button class="btn red" onclick="clearLog()">清除紀錄</button></section>
<section class="card"><label>設備 <select id="dev" onchange="loadLog()"><option value="">全部</option></select></label> <label>搜尋 <input id="q" oninput="debounceLoad()" placeholder="人名/事件/關鍵字"></label> <label>筆數 <input id="limit" value="500" style="width:80px"></label></section>
<section class="card"><div style="overflow:auto"><table><thead><tr><th>時間</th><th>設備</th><th>事件</th><th>內容</th><th>IP/數值</th></tr></thead><tbody id="rows"></tbody></table></div></section>
<section class="card"><h3>JSON</h3><pre id="status" class="status">loading...</pre></section>
</main>
<script>
function $(id){return document.getElementById(id)}
function esc(s){return String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));}
function label(t){return ({doorbell:'門鈴',door_open:'開門',command:'命令',command_ack:'命令完成',face_match:'人臉辨識',face_gate_auto_snapshot:'人臉快照',face_enroll:'人臉註冊',motion:'移動/人臉觸發',devices_save:'設備儲存',device_register:'設備上線',intercom_begin:'對講開始',intercom_end:'對講結束',events_clear:'清除紀錄'})[t]||t||'事件'}
async function j(url,opt){const r=await fetch(url,Object.assign({cache:'no-store'},opt||{}));const t=await r.text();try{return JSON.parse(t)}catch(e){return{ok:r.ok,status:r.status,raw:t}}}
let timer=null; function debounceLoad(){clearTimeout(timer);timer=setTimeout(loadLog,450)}
async function loadLog(){const d=await j('/api/devices');$('dev').innerHTML='<option value="">全部</option>'+(d.devices||[]).map(x=>'<option value="'+esc(x.id)+'">'+esc(x.id+' / '+(x.name||'')+' / '+(x.ip||''))+'</option>').join(''); const dev=new URLSearchParams(location.search).get('device_id')||$('dev').value; if(dev) $('dev').value=dev; const lim=$('limit').value||500; const q=encodeURIComponent($('q').value||''); const r=await j('/api/rt7/events/latest?limit='+lim+'&device_id='+encodeURIComponent($('dev').value||'')+'&q='+q); const ev=(r.events||[]).reverse(); $('rows').innerHTML=ev.map(e=>'<tr><td>'+esc(new Date(e.time||Date.now()).toLocaleString('zh-TW',{hour12:false}))+'</td><td>'+esc(e.device_id||e.device||'')+'</td><td><b>'+esc(label(e.type))+'</b><br><code>'+esc(e.type||'')+'</code></td><td>'+esc(e.message||e.summary||e.answer||e.name||'')+'</td><td>'+esc(e.ip||'')+' '+(e.confidence!==undefined?('相似度 '+esc(e.confidence)+'%'):'')+' '+(e.bytes?('bytes '+esc(e.bytes)):'')+'</td></tr>').join('')||'<tr><td colspan="5">無紀錄</td></tr>'; $('status').textContent=JSON.stringify(r,null,2);}
async function clearLog(){if(!confirm('確定清除事件紀錄？'))return; const r=await j('/api/rt7/events/clear',{method:'POST'}); $('status').textContent=JSON.stringify(r,null,2); loadLog();}
