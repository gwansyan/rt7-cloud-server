...
const SERVER_VERSION = 'RT7_WEBRTC_PHASE_B10A_ROLLBACK_NO_RESET_FIX';
...
// B10A: server-side audio priority window. While phone PCM is active, do not waste Railway
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (data, isBinary) => {
    try {
      // B10A: keeps B7 text/binary split; audio priority handled on ESP32 as Buffer too, with isBinary=false.
      // B7 fixed text/binary split; B10A keeps this behavior
      ...
      if (isBinary) {
        ...
        // B7: classify phone PCM by explicit role OR by small binary PCM length.
      ...
      if (msg && msg.role) {
        ...
        ws.send(JSON.stringify({ ok:true, type:'role_ack', phase:'B10A', role:ws.rt7Role, pcm_role:ws.rt7PcmRole||'', pcm_client:!!ws.rt7PcmClient, version:SERVER_VERSION, time:nowIso(), intercom_ws:rt7IntercomWsState_() }));
      }
...
<div class="wrap">
  <div class="card">
    <h1>RT7_WEBRTC_PHASE_B10A_ROLLBACK_NO_RESET_FIX</h1>
    ...
    <button id="btnStart">開始 Phase B10A Rollback No Reset Fix</button>
    <button id="btnStop" class="stop">停止測試</button>
    <button id="btnStatus" class="secondary">讀取 Railway 狀態</button>
  </div>
  <div class="card">
...
(function(){
  ...
  async function start(){
    ...
    ws=new WebSocket(wsUrl()+'?role=phone_pcm&device_id=%231&phase=B10A');
    ws.binaryType='arraybuffer';
    ws.onopen=function(){
      log('WS_STATE=OPEN');
      try{ ws.send(JSON.stringify({role:'phone_pcm', type:'role', phase:'B10A', device_id:'#1', time:Date.now()})); }catch(e){}
      try{ ws.send(JSON.stringify({type:'intercom_begin', role:'phone_pcm', phase:'B10A', device_id:'#1', time:Date.now()})); }catch(e){}
    };
  ...
  function stop(){
    running=false;
    try{ if(ws && ws.readyState===1) ws.send(JSON.stringify({type:'intercom_end', phase:'B10A', time:Date.now()})); }catch(_){}
