...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_2G_LIVE_FRAME_FACE_NO_CAPTURE_FIX';
...
function rt7SendWsJsonToEsp_(obj) {
  ...
  try {
    ...
    console.log('[FACE_API][V52G][WS_CMD_RELAY] type=' + safeString(obj && obj.type) + ' sent=' + n + ' open=' + seen);
  } catch (e) { console.warn('[FACE_API][V52G][WS_CMD_RELAY_ERR] ' + String(e && e.message || e)); }
  return n;
}
function rt7Sleep_(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }
async function rt7ForceRealtimeSnapshot_() {
  // V5.2G: do NOT interrupt LAN MJPEG stream to capture/upload a separate snapshot.
  // Use the newest already-uploaded live frame as the recognition image.  This keeps
  // the phone preview smooth so the user can aim the camera.  If no fresh live frame
  // exists, stop before face match rather than using a stale frame.
  const startMs = Date.now();
  const requestId = 'face_live_' + startMs + '_' + Math.floor(Math.random()*1000);
  const FRESH_MS = 1600;
  let latest = null;
  for (let i = 0; i < 16; i++) { // wait up to about 1.6s for next live frame
    latest = rt7GetLatestWithMeta_();
    if (latest) {
      const isLiveFrame = latest.snap_source === 'ws_frame' || latest.snap_source === 'live_frame' || latest.snap_source === 'raw_post' || latest.snap_source === 'json_b64' || latest.snap_source === 'realtime_snapshot';
      const freshByAge = latest.snap_age_ms !== null && latest.snap_age_ms <= FRESH_MS;
      if (latest.bytes >= 800 && isLiveFrame && freshByAge) {
        ...
        latest.snap_no_capture = true;
        console.log('[FACE_API][V52G][LIVE_FRAME_OK] request_id=' + requestId + ' wait_ms=' + latest.snap_wait_ms + ' bytes=' + latest.bytes + ' hash=' + latest.snap_hash + ' age_ms=' + latest.snap_age_ms + ' source=' + latest.snap_source);
        return latest;
      }
    }
    await rt7Sleep_(100);
  ...
  console.warn('[FACE_API][V52G][LIVE_FRAME_TIMEOUT] request_id=' + requestId + ' wait_ms=' + (Date.now() - startMs) + ' last_hash=' + (stale && stale.snap_hash || '') + ' last_age_ms=' + (stale && stale.snap_age_ms || '') + ' last_source=' + (stale && stale.snap_source || ''));
  return {
    ...
    summary: '未取得新鮮 live frame，已停止：不觸發 ESP32 拍照，避免影像串流變慢。'
  };
}
...
async function rt7FaceMatchLatest_() {
  console.log('[FACE_API][V52G] /api/rt7/face/match ENTER detect_first=1 force_realtime=1');
  ...
  if (!latest || latest.realtime_failed || !latest.b64) {
    ...
    console.warn('[FACE_API][V52G] stop before AI: NO_REALTIME_SNAPSHOT ws_sent=' + fail.snap_request_ws_sent + ' wait_ms=' + fail.snap_wait_ms + ' stale_hash=' + fail.snap_hash + ' stale_age_ms=' + fail.snap_age_ms);
    return fail;
  }
  try { fs.writeFileSync(FACE_DEBUG_SNAPSHOT_FILE, Buffer.from(latest.b64, 'base64')); } catch (e) { console.warn('[FACE_API][V52G] save face debug snapshot failed', e && e.message || e); }
  ...
  if (cloudState.face_gate_enabled && !gate.pass) {
    ...
    console.log('[FACE_API][V52G] FACE_GATE_SKIP hash=' + latest.snap_hash + ' reason=' + gate.reason);
    return skip;
  }

  const detect = await rt7DetectFaceOnly_(latest);
  console.log('[FACE_API][V52G] detect face_found=' + detect.face_found + ' count=' + detect.face_count + ' box=' + JSON.stringify(detect.face_box) + ' ratio=' + detect.face_ratio + ' reason=' + detect.reason + ' hash=' + latest.snap_hash);
...
app.get('/api/rt7/face/snapshot_trigger_test', (req,res)=>{
  ...
  const wsSent = rt7SendWsJsonToEsp_({ type:'face_snapshot_now', command:'face_snapshot_now', request_id:requestId, phase:'V52G', manual:true, time:nowIso() });
