...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V4_9T_ASYNC_DOOROPEN_BYPASS';
...
// V4.9T product system status: one endpoint for user support and maintenance.
app.get('/api/rt7/system/status', (req, res) => {
  ...
  res.json({
    ...
    cleanup: 'V4.9TA_STABLE_CLEANUP',
    stable_base: 'V4.8F11/V4.9T',
    time: nowIso(),
...
<title>RT7 Cloud Original UI V4.9T</title>
<style>
...
(function(){
  ...
  function setDebug(t){ /* V4.9F: hidden debug; no UI repaint */ }
  ...
  function lanFastDoorOpen(){
    setAnswer('LAN 開門命令送出中...');
    setDebug('lan fast door open :8081');
    var url='http://'+ip+':8081/api/door/open_fast?_='+Date.now();
    // Use fetch no-cors first. If Android blocks active mixed content, fallback to Image GET.
    try{
      fetch(url,{mode:'no-cors',cache:'no-store'}).catch(function(){
        try{ var im=new Image(); im.src=url+'&img=1'; }catch(_){ }
      });
    }catch(e){ try{ var im2=new Image(); im2.src=url+'&img=1'; }catch(_){ } }
    setAnswer('LAN 開門已送出（ESP32 8081 快速通道）');
  }
  bind('btnOpenDoor', async function(){
    if(currentStreamMode==='LAN') { lanFastDoorOpen(); return; }
    setAnswer('雲端開門命令送出中...');
    try{ var r=await j('/api/rt7/door/open?device_id='+encodeURIComponent('#1')); setAnswer((r.note||r.message||'開門命令已送出')); setDebug('cloud door open '+JSON.stringify(r).slice(0,160)); }catch(e){ setAnswer('開門失敗：'+e.message); }
  });
