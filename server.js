...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_6M3_KEYPAD_RELEASE_SAFE_FIX';
...
<div class="wrap">
  ...
  <div id="status" class="status">ready V5.6M3 / ${esc(dev.id||'')} / ${esc(host)}</div>
</div>
...
(function(){
  ...
  function makeUrl(code, phase, rawKey){
    return 'http://'+h+':8081/api/keypad?key='+encodeURIComponent(code)+'&phase='+encodeURIComponent(phase||'')+'&raw='+encodeURIComponent(rawKey||'')+'&tag=rt7_gpio_press_release_m3&_='+Date.now();
  }
  function fireImage(url){
    var img=new Image();
    img.onload=function(){};
  ...
  function sendCode(code, phase, rawKey){
    var url=makeUrl(code, phase, rawKey);
    fireImage(url);
    setStatus('KEY '+(phase||'send')+' '+(rawKey||'')+' -> '+code+' sent');
  }
  function sendRelease(rawKey){
    var url=makeUrl('99','release',rawKey);
    // release must be fastest: image beacon first, fetch keepalive as backup when browser allows it
    fireImage(url);
    try{
      if(window.fetch){
    ...
    setStatus('RELEASE '+rawKey+' -> 99');
  }
  function pressKey(rawKey, id){
    rawKey=String(rawKey||'').trim();
    if(!rawKey || activeKey){ return; }
    activeKey=rawKey;
    activeId=id==null?'x':id;
  ...
  function releaseKey(id){
    if(!activeKey){ return; }
    if(id!=null && activeId!=null && activeId!=='x' && id!==activeId){ return; }
    var k=activeKey;
    activeKey=null;
    activeId=null;
    var now=Date.now();
    if((now-lastReleaseAt)<70){ return; }
    lastReleaseAt=now;
    sendRelease(k);
  }

  document.querySelectorAll('.key[data-key]').forEach(function(b){
    var raw=function(){return b.getAttribute('data-key')||b.textContent.trim();};
    ...
    // desktop fallback; suppress synthetic mouse/pointer events after touch
    ...
    b.addEventListener('pointercancel', function(ev){
      if(Date.now()-lastTouchAt<700){ return; }
      ev.preventDefault();
      releaseKey(ev.pointerId);
    }, {passive:false});
