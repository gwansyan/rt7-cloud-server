...
<title>RT7 Cloud Original UI V4.9M</title>
<style>
...
(function(){
  ...
  function lan(){ videoWanted=true; currentStreamMode='LAN'; clearLanReconnect(); lanRetryCount=0; if(badge) badge.textContent='LAN'; if(empty) empty.innerHTML='內網直連 ESP32 流暢影像<br><span class="small">'+ip+'</span>'; if(img){ img.onerror=function(){ if(!videoWanted || currentStreamMode!=='LAN') return; lanRetryCount++; setAnswer('LAN 串流暫停，5 秒後重連'); setDebug('lan onerror retry='+lanRetryCount); clearLanReconnect(); lanReconnectTimer=setTimeout(function(){ if(!videoWanted || currentStreamMode!=='LAN') return; try{ img.src='about:blank'; }catch(e){} setTimeout(function(){ if(videoWanted && currentStreamMode==='LAN'){ img.src='http://'+ip+'/api/camera/stream?_lan_re='+Date.now(); } },250); },5000); }; img.onload=function(){ lanRetryCount=0; setDebug('lan mjpeg loaded'); }; try{ img.src='about:blank'; }catch(e){} setTimeout(function(){ if(videoWanted && currentStreamMode==='LAN'){ img.src='http://'+ip+'/api/camera/stream?_lan='+Date.now(); } },120); } setAnswer('內網直連影像模式'); setDebug('lan stream '+ip); }
