...
</style></head><body><div class="top"><a href="/rt7_dvr_ai_platform">← DVR平台</a><h1>RT7 V6.2J iCATCH Direct LAN Fix</h1><a href="/api/rt7/dvr/bridge/status">JSON</a></div><div class="wrap"><div class="notice"><b>V6.2J：</b>修正 LAN 直連 MJPEG_ERROR：Bridge 本機頁改為穩定輪詢最新 JPEG，並保留 MJPEG 備用。手機同 LAN 請直接開 Bridge 電腦網址，例如 http://192.168.0.55:8787/direct。CH02~CH04 仍停用。</div><div class="grid">${card}${disabled}</div><div class="card"><h2>Bridge 狀態</h2><div id="log" class="log">讀取中...</div></div></div><script>
...
function useLanDirect(){const input=document.getElementById('lanUrl'); let base=(input&&input.value||'').trim().replace(/\/+$/,''); if(!base){alert('請輸入 Bridge 電腦網址，例如 http://192.168.0.55:8787'); return;} localStorage.setItem('rt7_lan_bridge_url',base); location.href=base+'/direct';}
