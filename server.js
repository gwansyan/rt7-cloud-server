...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V4_8B_COMPARE_OVERLAY_REMOVE_FIX';
...
app.get('/rt7_stream_compare_test', (req, res) => {
  res.type('html').send(`<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>RT7 V4.8B Stream Compare</title>
<style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:#f3f6f8;color:#17262a;font-family:system-ui,-apple-system,"Noto Sans TC","Microsoft JhengHei",Arial,sans-serif}body{position:static!important;overflow:auto!important}.top{background:#0d2a30;color:#fff;padding:16px;text-align:center;font-weight:900}.wrap{max-width:980px;margin:0 auto;padding:12px}.card{position:relative!important;z-index:10!important;background:#fff;border:1px solid #d7dee5;border-radius:14px;padding:14px;margin:12px 0;box-shadow:0 2px 10px rgba(0,0,0,.05)}.controls{position:relative!important;z-index:999!important}.row{display:flex;gap:8px;flex-wrap:wrap;align-items:center}button{position:relative!important;z-index:1000!important;border:0;border-radius:12px;padding:12px 14px;margin:4px;color:#fff;font-weight:900;font-size:16px;touch-action:manipulation;pointer-events:auto!important}.blue{background:#1583d8}.green{background:#16a34a}.red{background:#dc2626}.gray{background:#475569}.orange{background:#d97706}select,input{position:relative!important;z-index:1000!important;font-size:16px;padding:10px;border:1px solid #9aa8b4;border-radius:10px;width:100%;box-sizing:border-box;margin:5px 0;background:#fff;pointer-events:auto!important}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.videoPanel{position:relative!important;z-index:1!important;background:#000;min-height:260px;border-radius:12px;overflow:hidden;display:flex;align-items:center;justify-content:center}.videoPanel img{display:block;width:100%;height:100%;min-height:260px;object-fit:contain;pointer-events:none!important;position:relative!important;z-index:1!important}.emptyText{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#fff;pointer-events:none!important;z-index:2}.label{font-weight:900;margin:8px 0;color:#7a2a19}.small{font-size:13px;color:#64748b;line-height:1.6}pre{white-space:pre-wrap;background:#0b1220;color:#cbd5e1;border-radius:12px;padding:10px;max-height:260px;overflow:auto}.ok{color:#16a34a;font-weight:900}.warn{color:#d97706;font-weight:900}.debug{font-size:12px;color:#64748b;margin-top:6px}@media(max-width:720px){.grid{grid-template-columns:1fr}.videoPanel{min-height:220px}.videoPanel img{min-height:220px}button{font-size:15px;padding:11px 12px}}
</style></head><body><div class="top">RT7 V4.8B 內網 / 外網影像串流比較測試</div><div class="wrap">
...
<div class="card controls"><label><b>選擇/輸入 ESP32 IP</b></label><select id="deviceSel"><option value="">手動輸入</option></select><input id="espIp" value="192.168.0.179" placeholder="例如 192.168.0.179"><div class="row"><button type="button" class="blue" id="btnLoadDevices">重新讀取設備</button><button type="button" class="green" id="btnSaveIp">套用 IP</button><button type="button" class="gray" id="btnLoadState">讀取狀態</button></div><div class="debug" id="debugTap">UI ready</div></div>
<div class="card controls"><div class="row"><button type="button" class="orange" id="btnLan">1. 內網直連 ESP32 串流</button><button type="button" class="green" id="btnCloud">2. Railway 雲端串流</button><button type="button" class="blue" id="btnBoth">同時比較</button><button type="button" class="red" id="btnStop">停止</button></div><div class="small">手機若不在同一 Wi‑Fi，內網直連會失敗。一般使用者外網仍走 Railway。</div></div>
<div class="grid"><div class="card"><div class="label">內網直連 ESP32 /api/camera/stream</div><div class="videoPanel"><img id="lanImg" alt="LAN stream"><div id="lanEmpty" class="emptyText">尚未開始</div></div><div id="lanInfo" class="small"></div></div><div class="card"><div class="label">外網 / Railway /api/rt7/camera/stream.mjpg</div><div class="videoPanel"><img id="cloudImg" alt="Cloud stream"><div id="cloudEmpty" class="emptyText">尚未開始</div></div><div id="cloudInfo" class="small"></div></div></div>
<div class="card"><b>判讀方式</b><div class="small">A. 內網直連順、Railway 慢：ESP32 攝影機正常，雲端 relay 是瓶頸。<br>B. 內網也慢：需調 ESP32 camera frame size / jpeg quality / Wi‑Fi。<br>C. 內網不能開但 Railway 可開：手機不在同 Wi‑Fi 或瀏覽器擋 HTTP 私網影像。</div></div>
...
function tap(s){ $('debugTap').textContent=s+' @ '+new Date().toLocaleTimeString(); log(s); }
...
function saveIp(){ espIp=($('espIp').value||$('deviceSel').value||espIp).trim().replace(/^https?:\/\//,'').replace(/\/.*$/,''); $('espIp').value=espIp; tap('ESP32 IP = '+espIp); }
async function loadDevices(){ tap('讀取設備'); try{const d=await j('/api/devices'); const devs=d.devices||[]; $('deviceSel').innerHTML='<option value="">手動輸入</option>'+devs.map(x=>'<option value="'+(x.ip||'')+'">'+(x.id||'')+' / '+(x.name||'')+' / '+(x.ip||'')+'</option>').join(''); if(devs[0]&&devs[0].ip){espIp=devs[0].ip;$('espIp').value=espIp;} log(d);}catch(e){log('loadDevices err '+e);} }
async function loadState(){ try{const s=await j('/api/rt7/camera/stream/state'); log(s);}catch(e){log('state err '+e);} }
function startLan(){ saveIp(); $('lanEmpty').style.display='none'; $('lanImg').onerror=()=>{$('lanInfo').innerHTML='<span class="warn">內網直連失敗：請確認同 Wi‑Fi，或瀏覽器是否擋 HTTP 私網影像。</span>';}; $('lanImg').onload=()=>{$('lanInfo').innerHTML='<span class="ok">內網直連已啟動。</span> 不經 Railway。';}; $('lanImg').src='http://'+espIp+'/api/camera/stream?_lan='+Date.now(); $('lanInfo').textContent='連線中： http://'+espIp+'/api/camera/stream'; tap('start LAN'); }
...
async function stopAll(){ $('lanImg').removeAttribute('src'); $('lanImg').src='about:blank'; $('cloudImg').removeAttribute('src'); $('cloudImg').src='about:blank'; $('lanEmpty').style.display='flex'; $('cloudEmpty').style.display='flex'; try{await j('/api/rt7/camera/stream/stop');}catch(e){} tap('stopped'); }
function bind(){ $('btnLoadDevices').onclick=loadDevices; $('btnSaveIp').onclick=saveIp; $('btnLoadState').onclick=loadState; $('btnLan').onclick=startLan; $('btnCloud').onclick=startCloud; $('btnBoth').onclick=startBoth; $('btnStop').onclick=stopAll; $('deviceSel').onchange=()=>{ if($('deviceSel').value){$('espIp').value=$('deviceSel').value;saveIp();} }; document.addEventListener('click',e=>{ if(e.target&&e.target.tagName==='BUTTON') tap('click '+e.target.id); }, true); }
bind(); loadDevices().then(loadState); setInterval(loadState,5000);
</script></body></html>`);
});
