...
const SERVER_VERSION = 'RT7_CLOUD_SERVER_V5_1_RESTORE_PHONE_INTERCOM';
...
(function(){
  ...
  async function intercomDown(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} if(talking)return; try{ await ensurePhoneMic(); await startEspRx(); await apiGetAudio('/api/ind_full/audio/phone_begin','phone_begin'); talking=true; txBytes=[]; sendQueue=[]; hpLastX=0; hpLastY=0; var b=document.getElementById('btnEndTalk'); if(b)b.classList.add('talking'); setAnswer('對講中：請按住說話'); setDebug('PHONE_TX start'); }catch(e){ setAnswer('手機麥克風啟用失敗：'+e.message); setDebug('intercom start failed'); } }
  async function intercomUp(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} if(!talking)return; talking=false; flushTxTail(); var b=document.getElementById('btnEndTalk'); if(b)b.classList.remove('talking'); await apiGetAudio('/api/ind_full/audio/phone_end','phone_end'); setAnswer('對講已結束'); setDebug('PHONE_TX stop'); }
  function bindIntercomPtt(){ var b=document.getElementById('btnEndTalk'); if(!b)return; b.addEventListener('pointerdown',intercomDown,false); b.addEventListener('pointerup',intercomUp,false); b.addEventListener('pointercancel',intercomUp,false); b.addEventListener('pointerleave',intercomUp,false); b.addEventListener('touchstart',intercomDown,{passive:false}); b.addEventListener('touchend',intercomUp,{passive:false}); }
...
const NODE_RED_MAPPING = [
  ...
  { group:'09 Intercom Audio', status:'restored-v5.1', nodered:'/api/ind_full/audio/* local proxy to ESP32 audio endpoints', railway:'/api/ind_full/audio/* returns compatibility JSON / queue scaffold', test:'Call begin/end endpoints; later add WebSocket PCM bridge one step at a time' }
];
