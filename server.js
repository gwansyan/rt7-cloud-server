...
function rt7DvrServeMjpeg_(req, res, id) {
  ...
  res.writeHead(200, {
    ...
    'Expires':'0',
    'X-Accel-Buffering':'no',
    'X-RT7-DVR-MJPEG':'V6_2F_ICATCH_LOW_LATENCY_ONLINE_FIX',
    'X-RT7-DVR-Camera': id
  });
  let closed = false;
  let busy = false;
...
function rt7IcatchRealtimeMjpegPage_() {
  ...
  const esc = (v) => String(v === undefined || v === null ? '' : v).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const card = `<div class="cam live"><div class="camTop"><b>${esc(ch1.id)} / ${esc(ch1.name||'CH01 大門')}</b><span id="s_CH01">讀取中</span></div><div class="view"><img id="img_CH01" src="/api/rt7/dvr/bridge/stream/CH01?fps=6&v=62f&first=${Date.now()}" onerror="document.getElementById('s_CH01').textContent='MJPEG_ERROR';"></div><div class="meta">True MJPEG: <code>/api/rt7/dvr/bridge/stream/CH01?fps=6</code><br><span id="m_CH01">MJPEG 串流中；每 3 秒檢查 Bridge 狀態，不重載畫面。</span></div><div class="btns"><button onclick="reloadMjpeg()">重連串流</button><button onclick="latestStill()">看單張</button><button onclick="ai('CH01')">AI 辨識</button></div></div>`;
...
</style></head><body><div class="top"><a href="/rt7_dvr_ai_platform">← DVR平台</a><h1>RT7 V6.2F iCATCH 低延遲真串流</h1><a href="/api/rt7/dvr/bridge/status">JSON</a></div><div class="wrap"><div class="notice"><b>V6.2F：</b>修正 ONLINE/OFFLINE 誤判，Bridge 影像 30 秒內都視為連線中；MJPEG 端點保留上一張有效影像，不因短暫延遲顯示 OFFLINE。Bridge 也加入 FFmpeg 低延遲參數，改善與 DVR 原畫面約 1~2 秒的延遲。CH02~CH04 仍停用。</div><div class="grid">${card}${disabled}</div><div class="card"><h2>Bridge 狀態</h2><div id="log" class="log">讀取中...</div></div></div><script>
...
function reloadMjpeg(){const img=document.getElementById('img_CH01'); if(img){img.src='/api/rt7/dvr/bridge/stream/CH01?fps=6&v=62f&reload='+Date.now(); setS('RECONNECT','ok');}}
